...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        // 🔥 Create contexts ONCE per request
        [stealthContext, nonStealthContext] = await Promise.all([
            getStealthContext(),
            getNonStealthContext()
        ]);
        ...
        const results = await Promise.allSettled([
            ...
            blinkitSearch(stealthContext, location_info, query),
            zeptoSearchItems(stealthContext, location_info, query)
        ]);
        console.log(results);
        ...
        if (nonStealthContext) await nonStealthContext.close();
    }
});
